...
namespace DXGridSample {
	public class MyLookUpEdit: PopupBaseEdit {
		...
		protected override void OnEditBoxTextChanged(object sender, TextChangedEventArgs e) {
			GridSearchString = ((MyEditStrategy)EditStrategy).GetEditTextPublic();
		...
		protected virtual void OnGridControlPreviewMouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e) {
			if (LayoutHelper.FindParentObject<GridRow>((DependencyObject)e.OriginalSource) != null) {
	...
	public class MyEditStrategy: ButtonEditStrategy {
		MyLookUpEdit LookUp;
		...
		protected override object GetEditValueInternal() {
			string containersValue = GetEditText();
		...
		public string GetEditTextPublic() {
			return GetEditText();
		}
		protected override void SyncWithEditorInternal() {
			...
			if (editValue == LookUp.GetEditValueByDisplayValue(GetEditValueInternal())) {
